...
> Canonical reference for OneCipher chain families, identifiers, derivation paths, and address formats.

## Identifier Types

OneCipher uses [CAIP](https://chainagnostic.org/) identifiers throughout. All wallet files, policy contexts, audit logs, and API parameters use these canonical formats — never shorthand aliases.
...
OneCipher groups chains into families that share a cryptographic curve and address derivation scheme. A single mnemonic derives accounts for all families.
...
| Nano | ed25519 | 165 | `m/44'/165'/{index}'` | `nano_` + base32 | `nano` |
...
Implementations MAY ship convenience endpoint defaults, but those defaults are deployment choices rather than OneCipher interoperability requirements.
...
4. Define the signing and message-signing behavior required by `signing-interface.md`.
5. Document any transaction serialization rules needed to produce deterministic signatures.

No changes to OneCipher core, the signing interface, or the policy engine are needed.
