...
> The core operations exposed by an OneCipher implementation: signing, sending, and message signing.
...
Current OneCipher implementations accept **already-serialized transaction bytes encoded as hex**. OneCipher signs those bytes, and `signAndSend` implementations submit the signed payload using the transport required by the target chain.
