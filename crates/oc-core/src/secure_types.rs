//! Hardened-memory types (`Passphrase`, `UnlockToken`).
//!
//! Split out of `types.rs` so that the `oc-crypto` dependency is confined to
//! a single optional module. `oc-core` is depended on by 13 crates, most of
//! which never touch hardened memory; keeping `oc-crypto` (and its `libc` /
//! `windows-sys` / `ml-dsa` subtree) behind the `hardened` feature keeps it
//! out of their dependency graphs entirely.
//!
//! Enabled via the `hardened` cargo feature (on by default for consumers that
//! request it explicitly: `oc-vault`, `oc-keyagent`, `oc-cli`).
...
impl Clone for UnlockToken {
    fn clone(&self) -> Self {
        // Best-effort mlock: Clone is infallible by trait contract, and an
        // mlock failure must never panic on a signing hot path. The fallback
        // copy is still zeroized on drop, just not page-locked.
        Self {
            key: HardenedBytes::from_slice_best_effort(self.key.as_ref()),
...
impl UnlockToken {
    ...
    ///
    /// Derives a 32-byte key with HKDF-SHA256 (RFC 5869):
    /// salt = `"onecipher-unlock-token-v2"`, IKM = key material,
    /// info = wallet ID. The wallet ID as HKDF `info` provides domain
