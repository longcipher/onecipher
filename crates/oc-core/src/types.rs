...
//!
//! Hardened-memory types (`Passphrase`, `UnlockToken`) live in
//! [`crate::secure_types`], behind the `hardened` feature, so that the
//! `oc-crypto` dependency does not leak into every `oc-core` consumer.
