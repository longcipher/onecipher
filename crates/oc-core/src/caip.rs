...
impl ChainId {
    fn validate_namespace(ns: &str) -> Result<(), OcError> {
        if ns.len() < 3 || ns.len() > 8 {
            return Err(OcError::CaipParseError {
                message: format!("namespace must be 3-8 characters, got {} ('{}')", ns.len(), ns),
            });
        }
    ...
    fn validate_reference(reference: &str) -> Result<(), OcError> {
        ...
        if !reference.chars().all(|c| c.is_ascii_alphanumeric() || c == '-' || c == '_') {
            return Err(OcError::CaipParseError {
