...
mod tests {
    ...
    fn test_policy_rule_serde_expires_at() {
        let rule = PolicyRule::ExpiresAt { timestamp: "2026-04-01T00:00:00Z".to_string() };
        let json = serde_json::to_value(&rule).unwrap();
    ...
    fn test_policy_serde_roundtrip() {
        let policy = Policy {
            ...
            rules: vec![
                ...
                PolicyRule::ExpiresAt { timestamp: "2026-12-31T23:59:59Z".into() },
            ],
            executable: None,
            config: None,
    ...
    fn test_policy_result_denied() {
        let result =
            PolicyResult::denied("spending-limit", "Daily spending limit exceeded: 0.95 / 1.0 ETH");
        assert!(!result.allow);
        assert_eq!(result.reason.as_deref(), Some("Daily spending limit exceeded: 0.95 / 1.0 ETH"));
    ...
    fn test_policy_context_typed_data_none_omitted() {
        let ctx = PolicyContext {
            ...
            spending: SpendingContext { daily_total: "0".into(), date: "2026-03-30".into() },
