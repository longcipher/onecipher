...
//!
//! Vectors: SpruceID `siwe` suite, see `vectors/siwe/SOURCE.txt`.
...
mod tests {
    ...
    fn expected_chain_id(name: &str, fields: &Value) -> String {
        ...
        let decimal =
            value.as_u64().map(|n| n.to_string()).or_else(|| value.as_str().map(str::to_owned));
        assert!(
            decimal.is_some(),
    ...
    fn assert_matches_fields(name: &str, parsed: &SiwxMessage, fields: &Value) {
        assert_eq!(parsed.scheme(), optional_str(fields, "scheme"), "{name}: scheme");
        assert_eq!(parsed.domain(), required_str(name, fields, "domain"), "{name}: domain");
        assert_eq!(parsed.address(), required_str(name, fields, "address"), "{name}: address");
        assert_eq!(parsed.uri(), required_str(name, fields, "uri"), "{name}: uri");
        assert_eq!(parsed.nonce(), required_str(name, fields, "nonce"), "{name}: nonce");
        assert_eq!(
            parsed.issued_at_raw(),
            required_str(name, fields, "issuedAt"),
            "{name}: issuedAt"
        );
        assert_eq!(parsed.chain_id(), expected_chain_id(name, fields), "{name}: chainId");
        assert_eq!(parsed.statement(), optional_str(fields, "statement"), "{name}: statement");
        assert_eq!(parsed.resources(), expected_resources(fields), "{name}: resources");
    }

    #[test]
    fn parsing_positive_from_str_round_trips() {
        let cases = load_object(PARSING_POSITIVE);
        assert_eq!(cases.len(), POSITIVE_COUNT, "parsing_positive.json case count");
    ...
    fn parsing_negative_from_str_fails() {
        ...
        assert_eq!(cases.len(), NEGATIVE_COUNT, "parsing_negative.json case count");
        assert!(
            cases.contains_key(SKIP_EIP55) && cases.contains_key(SKIP_CHAIN_ID),
            "vectors must contain `{SKIP_EIP55}` and `{SKIP_CHAIN_ID}`"
        );
