//! CAIP-122 signing-string parser ([`FromStr`] for [`SiwxMessage`]).
//!
//! Grammar (EIP-4361 / CAIP-122):
//!
//! ```text
...
const TAGGED_FIELDS: &[&str] =
    &[URI_TAG, VERSION_TAG, CHAIN_TAG, NONCE_TAG, IAT_TAG, EXP_TAG, NBF_TAG, RID_TAG];
...
impl FromStr for SiwxMessage {
    ...
    fn from_str(input: &str) -> Result<Self, Self::Err> {
        if input.len() > MAX_MESSAGE_BYTES {
            return Err(SiwxError::MessageTooLarge { len: input.len(), max: MAX_MESSAGE_BYTES });
        }
        ...
        let request_id =
            take_optional_tag(&mut lines, RID_TAG).map(|rid| check_request_id(&rid)).transpose()?;
...
fn take_optional_statement(lines: &mut Lines<'_>) -> Result<Option<String>, SiwxError> {
    ...
    if line.is_empty() {
        ...
        return match lines.peek() {
            ...
            _ => Err(SiwxError::invalid_format(FormatReason::MissingField(URI_TAG))),
        };
    }
    if is_tagged(line) {
        return Err(SiwxError::invalid_format(FormatReason::ExpectedBlankLine));
...
fn take_required_tag(lines: &mut Lines<'_>, tag: &'static str) -> Result<String, SiwxError> {
    let line =
        lines.peek().ok_or_else(|| SiwxError::invalid_format(FormatReason::MissingField(tag)))?;
    let value = line
        .strip_prefix(tag)
...
fn take_optional_ts(lines: &mut Lines<'_>, tag: &str) -> Result<Option<Timestamp>, SiwxError> {
    take_optional_tag(lines, tag).map(|s| Timestamp::parse(&s)).transpose()
}
...
fn next<'a>(lines: &mut impl Iterator<Item = &'a str>) -> Result<&'a str, SiwxError> {
    lines.next().ok_or_else(|| SiwxError::invalid_format(FormatReason::UnexpectedEof))
}
...
mod tests {
    ...
    fn sample() -> SiwxMessage {
        ...
        .expect("statement")
        .with_issued_at(ts("2021-09-30T16:25:24Z"))
        .expect("issued_at")
        .with_resources([
    ...
    fn roundtrip() {
        ...
        assert_eq!(parsed.to_sign_string("Ethereum"), text, "signing string must round-trip");
        assert_eq!(parsed.chain_name(), Some("Ethereum"));
        let mut expected = msg;
        expected.set_chain_name(Some("Ethereum".to_owned()));
    ...
    fn trailing_newline_fails() {
        ...
        assert!(matches!(
            err,
            SiwxError::InvalidFormat { reason: FormatReason::UnexpectedTrailing }
        ));
    }
    ...
    fn cr_in_input_fails() {
        ...
        assert!(matches!(err, SiwxError::InvalidFormat { reason: FormatReason::CrLf }));
    }

    #[test]
    fn missing_preamble_fails() {
        let err: SiwxError = "not a siwx message".parse::<SiwxMessage>().expect_err("should fail");
        assert!(matches!(err, SiwxError::InvalidFormat { reason: FormatReason::MissingPreamble }));
    }
    ...
    fn empty_chain_id_tag_is_format_empty_chain_id() {
        ...
        assert!(matches!(err, SiwxError::InvalidFormat { reason: FormatReason::EmptyChainId }));
    }
}
