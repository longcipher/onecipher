//! CAIP-122 error taxonomy.
//!
//! Three-way contract (mirrors the siwx design this crate absorbs):
//! malformed input -> `Invalid*`, cryptographic mismatch ->
//! [`SiwxError::VerificationFailed`], transport/RPC -> [`SiwxError::Backend`]
//! (reason only, never includes URLs).

use std::fmt;

/// Errors produced by Sign-In with X operations.
...
pub enum SiwxError {
    ...
    /// The `uri` field is not a valid URI.
    ...
    /// The `statement` field is empty or has illegal characters.
    ...
    /// The optional `request-id` is not `pchar` or exceeds the size limit.
    ...
    /// The message has expired (`expiration_time` in the past, beyond skew).
    #[error("message expired")]
    Expired,
    /// The message is not yet valid (`not_before` in the future, beyond skew).
    #[error("message not yet valid")]
    NotYetValid,
    /// `issued-at` is older than `max_issued_age` (beyond skew).
...
/// Why a message failed ABNF / layout parsing.
...
impl SiwxError {
    /// Convenience helper for [`SiwxError::InvalidFormat`].
...
mod tests {
    ...
    fn backend_display_is_reason_only() {
        let err = SiwxError::Backend { reason: "RPC timeout".to_owned() };
