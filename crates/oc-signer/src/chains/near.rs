...
mod tests {
    ...
    fn test_implicit_address_is_lowercase_hex_pubkey() {
        ...
        assert!(address.chars().all(|c| c.is_ascii_hexdigit() && !c.is_uppercase()));
    }
    ...
    fn test_encode_signed_transaction_layout() {
        ...
        let sig = SignOutput { signature: vec![0xAB; 64], recovery_id: None, public_key: None };
    ...
    fn test_encode_signed_transaction_rejects_wrong_sig_len() {
        let signer = NearSigner;
        let bad = SignOutput { signature: vec![0xAB; 32], recovery_id: None, public_key: None };
    ...
    /// 2. `encode_signed_transaction` emits exactly `tx_bytes || 0x00 || sig` (the canonical Borsh
    ///    layout for `SignedTransaction { transaction, signature: Signature::ED25519(_) }`).
    ///
    ...
    fn test_borsh_byte_parity_with_near_api_js_transaction1() {
        ...
        let dummy_sig =
            SignOutput { signature: vec![0xAB; 64], recovery_id: None, public_key: None };
        let signed = signer.encode_signed_transaction(&tx_bytes, &dummy_sig).unwrap();
