...
use xrpl::core::{
    binarycodec::{decode as xrpl_decode, encode as xrpl_encode},
...
mod tests {
    use super::*;
    use crate::{hd::HdDeriver, mnemonic::Mnemonic};
    ...
    fn test_sign_transaction_deterministic() {
        ...
        assert_eq!(sig1.signature, sig2.signature, "secp256k1 (RFC6979) must be deterministic");
    }
    ...
    fn test_sign_transaction_leading_zero_scalar_key() {
        ...
        assert!(result.is_ok(), "leading-zero-scalar key must sign: {:?}", result.err());
    ...
    fn test_sign_without_signing_pubkey_injects_it() {
        ...
        let blob = signer.encode_signed_transaction(&without_pubkey, &out).unwrap();
    ...
    fn test_encode_signed_transaction_matches_known_vector() {
        ...
        let encoded = signer.encode_signed_transaction(&tx_bytes, &sign_output).unwrap();
    ...
    fn test_encode_signed_transaction_invalid_tx_bytes() {
        ...
        let sign_output = SignOutput { signature: vec![0x30], recovery_id: None, public_key: None };
        let err = signer.encode_signed_transaction(tx_bytes, &sign_output).unwrap_err();
        assert!(
