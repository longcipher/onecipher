...
mod tests {
    ...
    fn test_sign_transaction_intent_digest() {
        ...
        let verifying_key = signing_key.verifying_key();
        let sig = ed25519_dalek::Signature::from_bytes(&result.signature.try_into().unwrap());
        verifying_key.verify(&digest, &sig).expect("signature should verify against intent digest");
    }
    ...
    fn test_sign_message_personal() {
        ...
        let verifying_key = signing_key.verifying_key();
        let sig = ed25519_dalek::Signature::from_bytes(&result.signature.try_into().unwrap());
        verifying_key.verify(&digest, &sig).expect("personal message signature should verify");
    }
    ...
    fn test_full_pipeline() {
        ...
        verifying_key.verify(&digest, &sig).expect("full pipeline signature should verify");
    }
