...
///
/// Fallible (M-07): a missing/unreadable `HOME` or any other store-open
/// failure is reported as `Err` with the underlying cause attached, instead of
/// panicking inside the signing thread. The daemon error channel only sees
/// clean `Err`s, so a panic here would bypass audit and teardown; returning
/// the error lets the connection loop reject the request and keep serving.
///
/// On failure nothing is cached — the next call retries initialization.
fn default_context() -> Result<&'static AgentContext, KeyAgentError> {
    if let Some(ctx) = DEFAULT_CONTEXT.get() {
        return Ok(ctx);
    }
    let ctx = AgentContext::open_default()
        .map_err(|e| KeyAgentError::Internal(format!("failed to initialize agent context: {e}")))?;
    // A racing thread may have won the `set`; the loser's freshly built
    // context is dropped unused and the shared winner instance is returned.
    let _ = DEFAULT_CONTEXT.set(ctx);
    DEFAULT_CONTEXT
        .get()
        .ok_or_else(|| KeyAgentError::Internal("agent context missing after initialization".into()))
}
...
/// `pending_challenges` map will be empty, so the client MUST have called
...
///
/// Fallible: returns `Err` both when the default context cannot be
/// initialized (e.g. `HOME` unavailable) and when the handler rejects the
/// request.
pub fn generate_passkey_challenge(credential_id: &str) -> Result<Vec<u8>, String> {
    ...
    let ctx = default_context().map_err(|e| e.to_string())?;
    let resp = handle_generate_challenge(ctx, &req).map_err(|e| e.to_string())?;
...
///
/// Fallible: returns `Err` both when the default context cannot be
/// initialized and when the proof is rejected.
pub fn authorize_passkey(auth: &crate::proto::PasskeyAuthorization) -> Result<(), String> {
    let ctx = default_context().map_err(|e| e.to_string())?;
    verify_passkey(ctx, auth).map(|_| ()).map_err(|resp| match resp.kind {
...
///
/// Fallible (M-07): returns `Err` if the default context cannot be
/// initialized (e.g. `HOME` unavailable). Callers MUST handle the `Err` —
/// ignoring it would silently leave the token unset and every SignAuth
/// request would be rejected as unauthorized.
pub fn set_sign_auth_internal_token(token: Option<Vec<u8>>) -> Result<(), KeyAgentError> {
...
/// and `Err(KeyAgentError)` for unrecoverable dispatcher-level failures:
/// an empty request with no `kind` set, or a default-context initialization
/// failure (M-07 — e.g. missing/unreadable `HOME`, with the cause attached).
/// Handler-internal errors are
...
fn handle_generate_challenge(
    ...
    // M-13: generation can be rejected with RateLimited when the verifier's
    // bounded pending-challenge map is full even after sweeping expired
    // entries — surface it as an error response, not a panic.
    let challenge = match verifier.generate_challenge() {
        Ok(c) => c,
        Err(e) => {
            return Ok(KeyAgentResponse::error(coded(
                err_code::AUTH,
                format!("challenge rejected: {e}"),
            )))
        }
    };
