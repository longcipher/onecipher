/// Thin helpers around `oc_core` for the payment layer.
///
/// All chain knowledge lives in `oc_core`. These are convenience
...
pub(crate) fn resolve_chain_type(network: &str) -> Option<ChainType> {
    // Fast path: known chain name or CAIP-2 ID.
    if let Ok(chain) = oc_core::parse_chain(network) {
...
pub(crate) fn display_name(network: &str) -> &str {
    if let Ok(chain) = oc_core::parse_chain(network) {
        return chain.name;
    }
    network
