...
const MOONPAY_CHAINS: &[(&str, MoonPayChain)] = &[
    ("base", MoonPayChain { display_name: "Base", moonpay_name: "base" }),
    ("ethereum", MoonPayChain { display_name: "Ethereum", moonpay_name: "ethereum" }),
    ("polygon", MoonPayChain { display_name: "Polygon", moonpay_name: "polygon" }),
    ("arbitrum", MoonPayChain { display_name: "Arbitrum", moonpay_name: "arbitrum" }),
    ("optimism", MoonPayChain { display_name: "Optimism", moonpay_name: "optimism" }),
    ("bsc", MoonPayChain { display_name: "BNB Chain", moonpay_name: "bnb" }),
    ("bnb", MoonPayChain { display_name: "BNB Chain", moonpay_name: "bnb" }),
    ("base-sepolia", MoonPayChain { display_name: "Base Sepolia", moonpay_name: "base-sepolia" }),
    ("solana", MoonPayChain { display_name: "Solana", moonpay_name: "solana" }),
];

const DEFAULT_MOONPAY_CHAIN: &MoonPayChain =
    &MoonPayChain { display_name: "Base", moonpay_name: "base" };

fn resolve_moonpay_chain(chain: Option<&str>) -> Result<&'static MoonPayChain, OcPayHttpError> {
    match chain {
        Some(name) => {
            let lower = name.to_lowercase();
            MOONPAY_CHAINS.iter().find(|(k, _)| *k == lower).map(|(_, v)| v).ok_or_else(|| {
                OcPayHttpError::new(
...
pub async fn fund(
    ...
    chain: Option<&str>,
    token: Option<&str>,
) -> Result<FundResult, OcPayHttpError> {
    ...
    let req = MoonPayDepositRequest {
        name: format!("OneCipher deposit ({token} on {})", mapping.display_name),
    ...
    Ok(FundResult {
        ...
        wallets: deposit.wallets.iter().map(|w| (w.chain.clone(), w.address.clone())).collect(),
...
pub async fn get_balances(
    wallet_address: &str,
    chain: Option<&str>,
) -> Result<Vec<TokenBalance>, OcPayHttpError> {
    let mapping = resolve_moonpay_chain(chain)?;
    let client = hpx::Client::new();
