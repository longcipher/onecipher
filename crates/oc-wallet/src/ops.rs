...
fn parse_chain(s: &str) -> Result<oc_core::Chain, OcWalletError> {
...
impl KeyPair {
    ...
    fn key_for_curve(&self, curve: oc_signer::Curve) -> &[u8] {
        match curve {
            oc_signer::Curve::Secp256k1 => &self.secp256k1,
            oc_signer::Curve::Ed25519 => &self.ed25519,
        }
    }
...
pub fn import_wallet_private_key(
    ...
    let keys = if let (Some(secp_hex), Some(ed_hex)) = (secp256k1_key_hex, ed25519_key_hex) {
        ...
        KeyPair { secp256k1: decode_hex_key(secp_hex)?, ed25519: decode_hex_key(ed_hex)? }
    } else {
        ...
        match source_curve {
            oc_signer::Curve::Secp256k1 => KeyPair {
                secp256k1: key_bytes,
                ed25519: ed25519_key_hex.map(decode_hex_key).transpose()?.unwrap_or(other_key),
            },
            oc_signer::Curve::Ed25519 => KeyPair {
                secp256k1: secp256k1_key_hex.map(decode_hex_key).transpose()?.unwrap_or(other_key),
                ed25519: key_bytes,
            },
        }
    };
...
#[cfg(feature = "rpc")]
fn http_post_json(url: &str, body: &str) -> Result<String, OcWalletError> {
...
mod tests {
    ...
    fn save_privkey_wallet(
        ...
        let keys = KeyPair { secp256k1: key_bytes, ed25519: ed_key };
