...
) -> Result<(String, ApiKeyFile), OcWalletError> {
    create_api_key_with_secret_permissions(
        name,
        wallet_ids,
        policy_ids,
        passphrase,
...
/// Create an API key with explicit secret-vault permissions (Phase 6).
pub fn create_api_key_with_secret_permissions(
    name: &str,
    wallet_ids: &[String],
    policy_ids: &[String],
    passphrase: &str,
...
pub fn sign_with_api_key(
    ...
    index: Option<u32>,
    vault_path: Option<&Path>,
) -> Result<crate::types::SignResult, OcWalletError> {
...
pub fn sign_typed_data_with_api_key(
    ...
    if let Some(domain_chain_id) = parsed.domain.get("chainId").and_then(parse_domain_chain_id) {
        let expected_chain_id = chain.evm_chain_id_u64().map_err(OcWalletError::InvalidInput)?;
    ...
    let typed_data_ctx = oc_core::policy::TypedDataContext {
        ...
        domain_name: parsed.domain.get("name").and_then(|v| v.as_str()).map(String::from),
        domain_version: parsed.domain.get("version").and_then(|v| v.as_str()).map(String::from),
        raw_json: typed_data_json.to_string(),
    };

    let context = oc_core::PolicyContext {
        chain_id: chain.chain_id.to_string(),
        wallet_id: wallet.id.clone(),
        api_key_id: key_file.id.clone(),
        transaction: oc_core::policy::TransactionContext {
            to: None,
            value: None,
            raw_hex: String::new(),
            data: None,
    ...
    if !result.allow {
        return Err(OcWalletError::Core(OcError::PolicyDenied {
...
pub fn enforce_policy_and_decrypt_key(
    ...
    index: Option<u32>,
    vault_path: Option<&Path>,
) -> Result<(SecretBytes, ApiKeyFile), OcWalletError> {
...
fn noop_spending_context(date: &str) -> oc_core::policy::SpendingContext {
    oc_core::policy::SpendingContext { daily_total: "0".to_string(), date: date.to_string() }
}

fn check_expiry(key_file: &ApiKeyFile) -> Result<(), OcWalletError> {
    if let Some(ref expires) = key_file.expires_at {
        let now = jiff::Timestamp::now();
        let exp = expires.parse::<jiff::Timestamp>().map_err(|e| {
            OcWalletError::Core(OcError::InvalidInput {
                message: format!("invalid expires_at timestamp '{}': {}", expires, e),
            })
        })?;
        if now > exp {
            return Err(OcWalletError::Core(OcError::ApiKeyExpired { id: key_file.id.clone() }));
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn create_api_key_and_verify_token() {
        ...
        assert!(token.starts_with(crate::key_store::TOKEN_PREFIX));
    ...
    fn create_api_key_accepts_wallet_name_and_stores_canonical_ids() {
        ...
        let chain = oc_core::parse_chain("base").unwrap();
    ...
    fn sign_with_api_key_wrong_chain_denied() {
        ...
        let (token, _) =
            create_api_key("agent", &[wallet_id], &[policy_id], passphrase, None, Some(&vault))
                .unwrap();

        // Try to sign on a chain NOT in the policy allowlist
        let chain = oc_core::parse_chain("ethereum").unwrap(); // eip155:1
        let tx_bytes = vec![0u8; 32];
    ...
    fn sign_with_api_key_expired_key_rejected() {
        ...
        assert!(result.is_err());
        match result.unwrap_err() {
            OcWalletError::Core(OcError::ApiKeyExpired { .. }) => {}
    ...
    fn sign_wallet_not_in_scope_fails() {
        ...
        oc_vault::save_encrypted_wallet(&wallet2, Some(&vault)).unwrap();
        ...
        let result =
            sign_with_api_key(&token, "other-wallet", &chain, &tx_bytes, None, Some(&vault));
    ...
    fn sign_typed_data_with_api_key_happy_path() {
        ...
        let (token, _) =
            create_api_key("td-agent", &[wallet_id], &[policy_id], passphrase, None, Some(&vault))
                .unwrap();
        let chain = oc_core::parse_chain("base").unwrap();
    ...
    fn sign_typed_data_with_api_key_non_evm_rejected() {
        ...
        let (token, _) =
            create_api_key("agent", &[wallet_id], &[policy_id], passphrase, None, Some(&vault))
                .unwrap();
        let chain = oc_core::parse_chain("solana").unwrap();
        let result = sign_typed_data_with_api_key(
            &token,
            "test-wallet",
            &chain,
    ...
    fn sign_typed_data_with_api_key_chain_mismatch_rejected() {
        ...
        let chain = oc_core::parse_chain("base").unwrap(); // eip155:8453
        let result = sign_typed_data_with_api_key(
            &token,
            "test-wallet",
            &chain,
