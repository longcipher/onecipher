...
/// Call a Nano RPC action via hpx and return the parsed JSON response.
fn nano_rpc_call(
    rpc_url: &str,
    ...
    rt.block_on(async {
        ...
        let resp =
            client.post(rpc_url).json(body).send().await.map_err(|e| {
                OcWalletError::BroadcastFailed(format!("Nano RPC call failed: {e}"))
            })?;
        ...
        if !resp.status().is_success() {
            ...
            return Err(OcWalletError::BroadcastFailed(format!(
                "Nano RPC call failed (HTTP {status}): {text}"
            )));
        }
        ...
        let resp_str = resp
            .text()
            .await
            .map_err(|e| OcWalletError::BroadcastFailed(format!("Nano RPC call failed: {e}")))?;
        ...
        if let Some(error) = parsed.get("error") {
            ...
            return Err(OcWalletError::BroadcastFailed(format!("Nano RPC error: {msg}")));
        }
...
pub fn account_info(
    rpc_url: &str,
    account: &str,
) -> Result<Option<NanoAccountInfo>, OcWalletError> {
    ...
    match nano_rpc_call(rpc_url, &body) {
        Ok(resp) => {
            let frontier = resp["frontier"]
                .as_str()
                .ok_or_else(|| {
                    OcWalletError::BroadcastFailed("no frontier in account_info response".into())
                })?
                .to_string();
            let balance = resp["balance"]
                .as_str()
                .ok_or_else(|| {
                    OcWalletError::BroadcastFailed("no balance in account_info response".into())
                })?
                .to_string();
            ...
            Ok(Some(NanoAccountInfo { frontier, balance, representative }))
        }
        Err(OcWalletError::BroadcastFailed(msg)) if msg.contains("Account not found") => Ok(None),
        Err(e) => Err(e),
    }
}
...
fn work_generate_single(
    ...
    resp["work"]
        .as_str()
        .map(|s| s.to_string())
        .ok_or_else(|| OcWalletError::BroadcastFailed("no work in work_generate response".into()))
}
...
pub fn work_generate(rpc_url: &str, hash: &str, difficulty: &str) -> Result<String, OcWalletError> {
    ...
    Err(last_error
        .unwrap_or_else(|| OcWalletError::BroadcastFailed("no PoW endpoints available".into())))
}
...
pub fn process_block(
    ...
    resp["hash"].as_str().map(|s| s.to_string()).ok_or_else(|| {
        OcWalletError::BroadcastFailed(format!("no hash in process response: {resp}"))
    })
}
