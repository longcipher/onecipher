...
pub(crate) fn create(file: &str) -> Result<(), CliError> {
    let contents = std::fs::read_to_string(file)
        .map_err(|e| CliError::InvalidArgs(format!("failed to read policy file '{file}': {e}")))?;

    let policy: oc_core::Policy = serde_json::from_str(&contents)
        .map_err(|e| CliError::InvalidArgs(format!("invalid policy JSON: {e}")))?;

    oc_wallet::policy_store::save_policy(&policy, None)?;
...
pub(crate) fn list() -> Result<(), CliError> {
    let policies = oc_wallet::policy_store::list_policies(None)?;
...
pub(crate) fn show(id: &str) -> Result<(), CliError> {
    ...
    if policy.rules.is_empty() {
        ...
        for rule in &policy.rules {
            let desc = match rule {
                oc_core::PolicyRule::AllowedChains { chain_ids } => {
                    format!("  allowed_chains: {}", chain_ids.join(", "))
                }
                oc_core::PolicyRule::ExpiresAt { timestamp } => {
...
pub(crate) fn delete(id: &str, confirm: bool) -> Result<(), CliError> {
    if !confirm {
