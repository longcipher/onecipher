...
pub(crate) fn run(purge: bool) -> Result<(), crate::CliError> {
    ...
    if !binary.exists() && !purge {
        println!("onecipher binary not found at {} — already uninstalled?", binary.display());
        return Ok(());
    }
    ...
    if install_dir.exists() && std::fs::read_dir(&install_dir).is_ok_and(|mut d| d.next().is_none())
    {
    ...
    println!();
    println!("onecipher has been uninstalled.");
    if !purge {
...
fn install_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("ONECIPHER_INSTALL_DIR") {
        ...
        home_dir().join(".onecipher/bin")
    }
}

fn vault_path() -> PathBuf {
    home_dir().join(".onecipher")
}
...
fn uninstall_node_bindings() {
    ...
    if let Ok(output) = check {
        if output.status.success() {
            ...
            let _ = Command::new("npm").args(["uninstall", "-g", "@onecipher/core"]).status();
        }
    }
}
...
fn uninstall_python_bindings() {
    ...
    let result = Command::new("pip3").args(["show", "onecipher"]).output();
    let installed = matches!(result, Ok(ref o) if o.status.success());

    if installed {
        println!("Removing Python bindings...");
        let _ = Command::new("pip3").args(["uninstall", "-y", "onecipher"]).status();
        return;
    }

    let result = Command::new("python3").args(["-m", "pip", "show", "onecipher"]).output();
    if let Ok(output) = result {
        if output.status.success() {
            println!("Removing Python bindings...");
            let _ = Command::new("python3")
                .args(["-m", "pip", "uninstall", "-y", "onecipher"])
                .status();
        }
    }
}
...
fn remove_path_entries(install_dir: &std::path::Path) {
    ...
    for rc in &rc_files {
        ...
        let filtered: Vec<&str> =
            contents.lines().filter(|line| !line.contains(dir_str.as_ref())).collect();
