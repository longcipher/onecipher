use oc_wallet::types::AccountInfo;
...
fn find_account_for_chain<'a>(
    ...
    accounts.iter().find(|a| a.chain_id.starts_with(chain_prefix)).ok_or_else(|| {
...
pub(crate) fn run(
    ...
    #[cfg(target_os = "macos")]
    {
        let _ = std::process::Command::new("open").arg(&result.deposit_url).spawn();
    }
    #[cfg(target_os = "linux")]
    {
        let _ = std::process::Command::new("xdg-open").arg(&result.deposit_url).spawn();
    }
...
pub(crate) fn balance(wallet_name: &str, chain: Option<&str>) -> Result<(), CliError> {
    let wallet = oc_wallet::get_wallet(wallet_name, None)?;
    let chain_name = chain.unwrap_or("base");
    ...
    for token in &balances {
        ...
        println!("{:>12.6} {:6} ${:<10.2}  {}", amount, token.symbol, value, token.name);
    }
